...
impl PlaybackManager {
    ...
    fn start_playback(
        ...
        {
            Ok(SampleFormat::F32) => {
                self.create_stream::<f32>(device, &config, audio_data, start_position, err_fn)
            }
            Ok(SampleFormat::I16) => {
                self.create_stream::<i16>(device, &config, audio_data, start_position, err_fn)
            }
            Ok(SampleFormat::U16) => {
                self.create_stream::<u16>(device, &config, audio_data, start_position, err_fn)
            }
